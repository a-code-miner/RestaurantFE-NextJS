...
const Navbar = () => {
    ...
    // بررسی اینکه آیا در داشبورد هستیم یا نه
    const isDashboard = pathname?.startsWith('/dashboards');

    return (
        <nav className={`flex flex-col lg:flex-row justify-between items-center bg-white/10 backdrop-blur-md border-b-cyan-200/20 shadow-sm h-auto lg:h-20 text-[18px] font-bold p-4 ${isDashboard ? 'relative' : 'fixed'} top-0 left-0 right-0 z-50 px-4 lg:px-20`}>
            ...
            <div className={`text-white mb-4 lg:mb-0 w-full lg:w-auto ${menuOpen ? 'block' : 'hidden'} lg:block`}>
                <ul className='flex flex-col lg:flex-row lg:space-x-8 lg:space-x-reverse items-center gap-2 lg:gap-0'>
                            ...
                            isLoggedIn ? (
                                <div className="relative group">
                                    ...
                                    <div className={`absolute right-0 w-full bg-white rounded-lg shadow-lg py-2 z-50 transition-all duration-300 ease-in-out ${menuOpen === 'profile' ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-95 pointer-events-none'} group-hover:opacity-100 group-hover:scale-100 group-hover:pointer-events-auto`}>
                                        ...
                                        <Link href={`/dashboards/customer/${user.id}`} className="block px-4 py-2 text-gray-700 hover:bg-gray-100">داشبورد</Link>
